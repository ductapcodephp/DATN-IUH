...
export default function Index() {
    const articles = [
        ...
        },
        {
            ...
            slug: "chatgpt-github-copilot-dev-thay-the",
    ...
    return (
        <FrontendLayout>
            <Head title="Blog Công Nghệ & Khởi Nghiệp" />

            <section className="hero-section text-center py-5 mb-4">
                <div className="container py-2">
                    <h1 className="hero-title">Blog Chia Sẻ Kiến Thức</h1>
                    <p className="hero-desc col-lg-6 mx-auto mb-0">Cập nhật tin tức công nghệ mới nhất, hướng dẫn lập trình thực chiến và cẩm nang định hướng sự nghiệp IT.</p>
                </div>
            </section>
            ...
            <div className="container mb-5">
                <div className="d-flex flex-wrap gap-2 justify-content-center border-bottom pb-3">
                    ...
                    <button 
                        onClick={() => setActiveTab("kinh nghiệm")} 
                        className={`btn btn-sm px-4 rounded-pill ${activeTab === "kinh nghiệm" ? "btn-dark" : "btn-outline-secondary border-0"}`}
                    >
                        Kinh nghiệm đi làm
                    </button>
                    <button 
                        onClick={() => setActiveTab("frontend")} 
                        className={`btn btn-sm px-4 rounded-pill ${activeTab === "frontend" ? "btn-dark" : "btn-outline-secondary border-0"}`}
                    >
                        Frontend Tips
                    </button>
                    <button 
                        onClick={() => setActiveTab("backend")} 
                        className={`btn btn-sm px-4 rounded-pill ${activeTab === "backend" ? "btn-dark" : "btn-outline-secondary border-0"}`}
                    >
                        Backend & Cloud
                    </button>
                    <button 
                        onClick={() => setActiveTab("ai")} 
                        className={`btn btn-sm px-4 rounded-pill ${activeTab === "ai" ? "btn-dark" : "btn-outline-secondary border-0"}`}
                    >
                        Xu hướng AI
                    </button>
                </div>
                            ...
                            <div className="col-12 col-md-6 col-lg-4" key={article.id}>
                                <div className="course-card h-100 d-flex flex-column cart-card-hover border rounded-3 overflow-hidden shadow-sm bg-white">
                                    <img 
                                        ...
                                        style={{ height: "200px" }}
                                        loading="lazy"
                                        onError={(e) => { e.target.src = "/assets/frontend/img/default-blog.png"; }}
                                    />
                                    <div className="course-body d-flex flex-column flex-grow-1 p-4">
                                        ...
                                        <h3 className="course-title fs-5 fw-bold mb-2 mt-1">
                                            <a href="#" className="text-dark text-decoration-none hover-text-accent">{article.title}</a>
                                        </h3>
