...
export default function CourseListBlock({ block, editable, courses, filters, categories, enrolledCourseIds }) {
    
    // Hardcoded data for CMS preview
    const isMock = editable || !courses;
    
    const mockCourses = {
        data: [
            {
                id: 1, title: 'Khóa học React JS Cơ bản', slug: '#', price: 500000, original_price: 1000000, 
                category: { name: 'Lập trình Web' }, students_count: 1500, reviews_avg_rating: 4.8,
                thumbnail: null
            },
            {
                id: 2, title: 'Làm chủ Laravel 11', slug: '#', price: 0, original_price: 0, 
                category: { name: 'Backend' }, students_count: 3200, reviews_avg_rating: 5.0,
                thumbnail: null
            },
            {
                id: 3, title: 'Master Node.js', slug: '#', price: 800000, original_price: 1200000, 
                category: { name: 'Backend' }, students_count: 800, reviews_avg_rating: 4.5,
                thumbnail: null
            }
        ],
        links: [], current_page: 1, last_page: 1, total: 3
    };

    const mockCategories = [
        { id: 1, name: 'Lập trình Web', slug: 'web' },
        { id: 2, name: 'Backend', slug: 'backend' },
    ];

    const safeCourses = isMock ? mockCourses : courses;
    const safeCategories = isMock ? mockCategories : categories;
    const safeFilters = isMock ? { search: "", category: [], price: "all", rating: "", sort: "newest" } : filters;
    
    const { data: courseList, links, current_page, last_page, total } = safeCourses;

    const { data, setData, get } = useForm({
        search: safeFilters.search || "",
        category: safeFilters.category || [],
        price: safeFilters.price || "all",
        rating: safeFilters.rating || "",
        sort: safeFilters.sort || "newest"
    });

    // We don't fetch auth data in preview mode to prevent errors
    const pageProps = usePage().props;
    const auth = isMock ? { wishlisted_course_ids: [] } : pageProps.auth;
                ...
                <div className="container">
                    ...
                    <nav aria-label="breadcrumb">
                        <ol className="breadcrumb mb-0 font-sm">
                            <li className="breadcrumb-item"><Link href={!isMock ? route('frontend.home') : '#'} className="text-muted">Trang chủ</Link></li>
                        ...
                        <div className="col-lg-3">
                            <div className="filter-sidebar">
                                <div className="d-flex justify-content-between align-items-center mb-3">
                                    ...
                                    <button onClick={() => !isMock && get(route('frontend.course.index'))} className="btn btn-sm btn-outline-dark fw-semibold">Làm mới</button>
                                </div>
                                        ...
                                        <div className="col-12 col-md-6 col-xl-4" key={course.id}>
                                            <div className="course-card position-relative">
                                                <button 
                                                    ...
                                                    onClick={(e) => {
                                                        e.preventDefault();
                                                        if(!isMock) router.post(route('frontend.wishlist.toggle'), { course_id: course.id }, { preserveScroll: true });
                                                    }}
                                                >
                                                    <i className={`fa-heart text-danger ${isWishlisted ? 'fa-solid' : 'fa-regular'}`}></i>
                                                </button>
                                              
                                            <Link href={!isMock ? route('frontend.course.detail', { slug: course.slug }) : '#'} className="text-decoration-none text-dark">
                                                <img 
