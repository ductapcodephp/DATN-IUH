...
export default function Wallet({ wallet, transactions, onlinePayments, filters }) {
    ...
    const handleWithdraw = (e) => {
        e.preventDefault();
        withdrawForm.post(route('finance.wallet.withdraw'), {
            onSuccess: () => {
            ...
            <div className="mb-4 d-flex justify-content-between align-items-center">
                ...
                <div className="d-flex gap-2">
                    <button 
                        ...
                        style={{ background: '#EA580C', borderRadius: '12px', border: 'none' }}
                    >
                        <i className="fa-solid fa-money-bill-transfer me-2"></i>Rút tiền
                    </button>
                    <button 
                        onClick={() => setShowDepositModal(true)}
                        className="btn fw-semibold text-white px-4" 
            ...
            {showWithdrawModal && (
                <div className="modal fade show" style={{ display: 'block', backgroundColor: 'rgba(0,0,0,0.5)' }} tabIndex="-1">
                    <div className="modal-dialog modal-dialog-centered">
                        <div className="modal-content" style={{ borderRadius: '16px', border: 'none' }}>
                            <div className="modal-header border-0 pb-0">
                                <h5 className="modal-title fw-bold" style={{ color: '#1F2937' }}>Rút tiền về ngân hàng</h5>
                                <button type="button" className="btn-close" onClick={() => setShowWithdrawModal(false)}></button>
                            </div>
                            <form onSubmit={handleWithdraw}>
                                <div className="modal-body">
                                    <div className="mb-3">
                                        <label className="form-label fw-semibold" style={{ fontSize: '0.875rem' }}>Số tiền cần rút (VNĐ)</label>
                                        <input 
                                            type="number" 
                                            className={`form-control form-control-lg ${withdrawForm.errors.amount ? 'is-invalid' : ''}`}
                                            ...
                                            style={{ borderRadius: '10px' }}
                                        />
                                        {withdrawForm.errors.amount && <div className="invalid-feedback">{withdrawForm.errors.amount}</div>}
                                        <div className="form-text">Tối thiểu 50,000đ. Lưu ý: Tiền sẽ được rút về tài khoản ngân hàng mặc định của bạn.</div>
                                    </div>
                                </div>
                                <div className="modal-footer border-0 pt-0">
                                    <button type="button" className="btn btn-light" onClick={() => setShowWithdrawModal(false)} style={{ borderRadius: '10px', fontWeight: 500 }}>Hủy</button>
                                    <button type="submit" className="btn text-white px-4" disabled={withdrawForm.processing} style={{ background: '#EA580C', borderRadius: '10px', fontWeight: 600 }}>
                                        {withdrawForm.processing ? 'Đang xử lý...' : 'Tạo yêu cầu rút tiền'}
                                    </button>
                                </div>
                            </form>
                        </div>
