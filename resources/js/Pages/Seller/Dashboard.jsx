...
export default function Dashboard({ stats, recentEnrollments, chartData, weeklyChartData, currentFilter }) {
    ...
    const CustomTooltip = ({ active, payload, label }) => {
        if (active && payload && payload.length) {
            return (
                <div style={{ background: '#fff', padding: '10px', border: '1px solid #ccc', borderRadius: '5px', boxShadow: '0 4px 6px rgba(0,0,0,0.1)' }}>
                    <p style={{ margin: 0, fontWeight: 'bold' }}>{label}</p>
                    <p style={{ margin: 0, color: 'var(--fire)' }}>Doanh thu: {formatCurrency(payload[0].value)}</p>
                </div>
            );
        }
        return null;
        ...
        <>
            ...
            <div className="page">
                <div className="page-header">
                    ...
                    <Link href={route('seller.courses.create')} className="btn-primary"><i className="fa-solid fa-plus"></i> Tạo khóa học mới</Link>
                </div>
                ...
                <div style={{ display: 'flex', gap: '20px', borderBottom: '1px solid var(--border)', marginBottom: '20px' }}>
                    <button 
                        ...
                        style={{ padding: '10px 0', border: 'none', background: 'none', cursor: 'pointer', fontWeight: activeTab === 'overview' ? 'bold' : 'normal', color: activeTab === 'overview' ? 'var(--fire)' : 'var(--muted)', borderBottom: activeTab === 'overview' ? '2px solid var(--fire)' : '2px solid transparent' }}
                    >
                        Tổng quan
                    </button>
                    <button 
                        onClick={() => setActiveTab('analytics')}
                        style={{ padding: '10px 0', border: 'none', background: 'none', cursor: 'pointer', fontWeight: activeTab === 'analytics' ? 'bold' : 'normal', color: activeTab === 'analytics' ? 'var(--fire)' : 'var(--muted)', borderBottom: activeTab === 'analytics' ? '2px solid var(--fire)' : '2px solid transparent' }}
                    >
                        Phân tích Doanh thu
                    </button>
                </div>
                ...
                <div style={{ display: activeTab === 'overview' ? 'block' : 'none' }}>
                    ...
                    <div className="stats-grid">
                        <div className="stat-card">
                            ...
                            <div className="stat-card-val">{formatCurrency(stats?.currentMonthRevenue)}</div>
                        ...
                        <div className="stat-card">
                            ...
                            <div className="stat-card-val">{new Intl.NumberFormat('vi-VN').format(stats?.totalStudents || 0)}</div>
                        ...
                        <div className="stat-card">
                            ...
                            <div className="stat-card-val">{stats?.activeCourses || 0}</div>
                        ...
                        <div className="stat-card">
                            ...
                            <div className="stat-card-val">{stats?.avgRating || 0}</div>
