...
export default function Edit({ pageData }) {
    ...
    const { data, setData, put, processing, errors } = useForm({
        ...
        sub_title: pageData.post?.sub_title || '',
        slug: pageData.post?.slug || '',
        description: pageData.post?.description || '',
        content: pageData.post?.content || '',
        thumbnail: pageData.post?.thumbnail || '',
        published: pageData.post?.published || 'publish',
        language: pageData.language || 'vi',
        tags: pageData.post?.tags || [],
        css: pageData.css || '',
        custom_css: pageData.custom_css || '',
        google_title: pageData.post?.social_sharing?.google_title || '',
        google_description: pageData.post?.social_sharing?.google_description || '',
        facebook_title: pageData.post?.social_sharing?.facebook_title || '',
        facebook_description: pageData.post?.social_sharing?.facebook_description || '',
        is_hot: pageData.post?.is_hot === 1,
        is_new: pageData.post?.is_new === 1,
        keep_slug: true
    });

    const handleSubmit = (e) => {
        e.preventDefault();
        put(route('cms.page.update', pageData.id));
    };

    return (
        <CMSLayout>
            <Head title="Cập nhật Trang - CMS" />
            
            <form onSubmit={handleSubmit}>
                <div className="d-flex justify-content-between align-items-center mb-5">
                    <div>
                        <h2 className="wow-title mb-1">Cập nhật Trang</h2>
                        <p className="m-0" style={{ color: 'var(--wow-text-muted)' }}>Chỉnh sửa thông tin trang</p>
                    </div>
